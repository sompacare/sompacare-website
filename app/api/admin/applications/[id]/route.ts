...
import { getApplication, markOnboardingSent, updateApplicationStatus } from "@/lib/supabase/admin";
...
export async function PATCH(
  ...
  try {
    ...
    let platformPlace: Awaited<ReturnType<typeof placeCareerApplicationOnPlatform>> | null = null;
    let platformHire: Awaited<ReturnType<typeof hireCareerApplicationOnPlatform>> | null = null;

    if (nextStatus === "placed" && existing.status !== "placed") {
      platformPlace = await placeCareerApplicationOnPlatform(id);
      ...
      } else if (!existing.onboarding_sent_at) {
    ...
    if (nextStatus === "hired" && existing.status !== "hired") {
      if (existing.status !== "placed" && updated.status !== "placed") {
        platformPlace = await placeCareerApplicationOnPlatform(id);
        if (platformPlace.placed) {
          try {
            await markOnboardingSent(id);
          } catch (error) {
            console.error("Unable to record onboarding_sent_at before hire:", error);
          }
        }
      }

      platformHire = await hireCareerApplicationOnPlatform(id);
      const refreshed = await getApplication(id);
      if (refreshed) application = refreshed;
    }

    return NextResponse.json({ application, onboarding, platformPlace, platformHire });
  } catch {
